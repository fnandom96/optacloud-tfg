...
public class mainDelay {
    ...
    private static DatacenterBrokerDelay broker;
    ...
    static private void init_variables() {
        opta = true;
        delayedSubmit = true; //activar execució múltiple
        //delay.add(2.0);
        //delay.add(4.0);
        delay.add(8.0);
        ...
        num_delayedcloudlets = 0;
    }

    private static DatacenterBrokerDelay createBroker(String name){

        DatacenterBrokerDelay broker = null;
        try {
            broker = new DatacenterBrokerDelay(name);
    ...
    static private void submitVMs() {
        ...
        for (i = 0; i < num_vms/2; ++i) {
            String vm_name = "VM_" + i;
            if (vm_scheduler.equals("CloudletSchedulerTimeShared")) sch = new CloudletSchedulerTimeShared();
            else sch = new CloudletSchedulerSpaceShared();
            vm[i] = new Vm(i, brokerId, vm_mips, 2, vm_ram, vm_bw, vm_size, vm_name, sch);
            list.add(vm[i]);
        }
        ...
        while (i < num_vms) {
            ...
            vm[i] = new Vm(i, brokerId, vm_mips, 7, vm_ram, vm_bw, vm_size, vm_name, sch);
            list.add(vm[i]);
            ++i;
        }
        ...
        for (int j = 0; j < submissions; ++j) {
            list = new LinkedList<>();
            vm = new Vm[num_delayedvms];
            for (int k = 0; k < num_delayedvms; ++k) {
                String vm_name = "VM_" + i;
                if (vm_scheduler.equals("CloudletSchedulerTimeShared")) sch = new CloudletSchedulerTimeShared();
                else sch = new CloudletSchedulerSpaceShared();
                vm[k] = new Vm(i, brokerId, vm_mips, 2, vm_ram, vm_bw, vm_size, vm_name, sch);
                for (int l = 0; l <= j; ++l) {
