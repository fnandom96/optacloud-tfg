...
import com.optacloud.domain.HostStrengthComparatorClass;
import com.optacloud.domain.VmDifficultyComparatorClass;
...
@PlanningEntity(difficultyComparatorClass = VmDifficultyComparatorClass.class)
public class Vm {
	...
	//Determina si la VM ha sigut creada recentment
	private boolean new_vm;

	//Historial dels host on ha estat la VM
	private List<Integer> host_history;

	//Temps on s'ha començat a executar la VM al host host_history[i]
	private List<Double> host_intervals;

	//Potencia consumida al host_history[i]
	...
	//OptaPlanner necessita un constructor sense arguments
	public Vm() {
	}
	...
	public Vm(
		...
		new_vm = true;
		host_intervals = new ArrayList<>();
		host_history = new ArrayList<>();
	 ...
	 * @return the host
	 */
	@PlanningVariable(valueRangeProviderRefs = {"hostRange"}, strengthComparatorClass = HostStrengthComparatorClass.class/*,nullable = true*/)
	public Host getHost() {
		return host;
	}
	...
	//Al allotjar la VM, el Broker comprova si és nova
	public boolean isNew() { return new_vm; }
	public void setNew(boolean new_vm) { this.new_vm = new_vm; }

	//Obtenir l'historial de hosts on s'ha executat la VM
	public List<Integer> getHostHistory() {
		return host_history;
	}

	//Afegir el host al qual ha estat assignat la VM
	public void addHostToHistory(int hostId) {
		host_history.add(hostId);
	}

	//Obtenir el temps en el que la VM s'assigna al host host_history[i]
	public List<Double> getHostIntervals() {
		return host_intervals;
	}

	//Afegir el temps on s'assigna al host de host_history[i]
	public void addHostInterval(double time) {
		host_intervals.add(time);
	}

	//Obtenir el temps en el que la VM s'assigna al host host_history[i]
	public List<Double> getPowerHistory() {
		return power_history;
	}

	//Afegir el temps on s'assigna al host de host_history[i]
	public void addPowerToHistory(double power) {
		power_history.add(power);
	}
