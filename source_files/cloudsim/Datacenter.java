...
public class Datacenter extends SimEntity {
	...
	//Llista de cloudlets que estan en proces de migracio d'una VM a una altre VM
	private List<Cloudlet> cloudlets_migrating;
	...
	public Datacenter(
		...
		cloudlets_migrating = new ArrayList<>();
	}
	...
	public void processEvent(SimEvent ev) {
		...
		// Resource characteristics inquiry
			...
			case CloudSimTags.MIGRATE_VM_ACK:
				...
				break;
			//custom
			case CloudSimTags.DEALLOCATE_VM_ACK:
				deallocateVM(ev, true);
				break;
			...
			case CloudSimTags.PAUSE_VMCLOUDLETS:
				stopVmCloudlets(ev);
				break;
	...
	//Desallotjar la vm seleccionada
	protected void deallocateVM(SimEvent ev, boolean ack) {
		Vm vm = (Vm) ev.getData();
		...
		/*for (Iterator<Vm> iter = getVmList().listIterator(); iter.hasNext(); ) {
			Vm vml = iter.next();
			if (vml.getId() == vm.getId()) {
				iter.remove();
			}
		}*/
		getVmList().remove(vm);

		Log.printConcatLine("Deallocation of VM#" + vm.getId() + " completed");
	}
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
            if (CloudSim.getOpta()) { //MOTOR OPTAPLANNER
                //rep la llista completa de vm
                List<Vm> vmList = (List<Vm>) ev.getData();
                
                //result[x] true si s'ha pogut allotjar la màquina a la posició x
                boolean[] result;
                result = getVmAllocationPolicy().allocateHostForVm(vmList);
                ...
                for (int i = 0; i < vmList.size(); ++i) {
                    ...
                    if (ack) {
						...
						//retornar al broker si s'ha allotjat la vm a un host
	...
	protected void processVmDestroy(SimEvent ev, boolean ack) {
		...
		//Desallotjar els recursos que utilitza la màquina
		//deallocateVM(ev, ack);
		getVmAllocationPolicy().deallocateHostForVm(vm);
		getVmList().remove(vm);
	}

	//Atura les els cloudlets que s'executan a la VM
	protected void stopVmCloudlets(SimEvent ev) {
		Vm vm = (Vm) ev.getData();
		//Afegir els cloudlets que encara s'executan de la VM a migrar
		//setCloudletsMigrating(vm.getCloudletScheduler().migrateCloudlets());
		vm.getCloudletScheduler().migrateCloudlets();
	}

	//S'intenta moure la VM al host especificat.
	protected void migrateVM(SimEvent ev, boolean ack) {
		Map<Vm, Integer> migrate = (Map<Vm, Integer>) ev.getData();

		Map.Entry<Vm,Integer> entry = migrate.entrySet().iterator().next();
		Vm vm = entry.getKey();
		int hostId = entry.getValue();
		Host host = getCharacteristics().getHostList().get(hostId);

		//Guardar l'anterior host per si no es pot migrar
		Host prev_host = vm.getHost();

		//Desallotjar
		if (prev_host != null) getVmAllocationPolicy().deallocateHostForVm(vm);
		...
		if (!result) {
			Log.printLine("Couldn't allocate VM#" + vm.getId() + " to Host#" + hostId + ". Returning to original state");
			if (prev_host != null) getVmAllocationPolicy().allocateHostForVm(vm, prev_host);
		} else {
			if (prev_host == null) {
				getVmList().add(vm);
			}
		...
		if (ack) {
			...
			data[0] = vm.getId();
			if (result) {
				data[1] = CloudSimTags.TRUE;
			} else {
				data[1] = CloudSimTags.FALSE;
			}
			if (prev_host == null) {
				data[2] = CloudSimTags.FALSE;
			} else {
			...
			sendNow(ev.getSource(), CloudSimTags.MIGRATE_VM_ACK, data);
		}
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		// get the cloudlet
		//------
		//les maquines antigues no s'estan reutilitzant
		//--- NO TROBA HOST ---- vmId potser ja no esta a cap host
		/*System.out.println("vmid: " + vmId);
		Host h = getVmAllocationPolicy().getHost(vmId, userId);
		if (getVmAllocationPolicy().getHost(vmId, userId) == null) {
		...
		//falla
		//Vm v = h.getVm(vmId,userId);
		//busca la VM
		if (getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId) == null) return;*/
		//------
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			//Afegir al historial la vm que l'executa
			cl.addVmToHistory(vmId);

			//Actualitzar estat
			if (cl.getMigrating()) {
				cl.setMigrating(false);
				getCloudletsMigrating().remove(cl);
			}
			...
			cl.addTimeIntervals(CloudSim.clock());
	...
	private List<Cloudlet> getCloudletsMigrating() { return cloudlets_migrating; }

	private void setCloudletsMigrating(List<Cloudlet> list) { this.cloudlets_migrating.addAll(list); }
