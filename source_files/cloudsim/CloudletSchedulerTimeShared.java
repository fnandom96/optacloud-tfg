...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	//implementada
	@Override
	public List<Cloudlet> migrateCloudlets() {
		List<Cloudlet> migrate = new ArrayList<>();
		for (ResCloudlet rgl : getCloudletExecList()) {
			rgl.getCloudlet().setMigrating(true);
			migrate.add(rgl.getCloudlet());
			rgl.finalizeCloudlet();
		}
		getCloudletExecList().removeAll(migrate);
		return migrate;
	}
